...
import EditTax from './create-tax';
import {
  Select,
  SelectContent,
  SelectItem,
...
import { addEntityActivity, deleteQuote, editQuote } from '@/app/lib/actions';
...
export default function EditQuoteForm({
  ...
  const initialTaxRate = quote.tax_id
    ? taxes.filter((tax) => tax.id === quote.tax_id)[0].tax_rate
    : 0;

  const [loading, setLoading] = useState(false);
  const [selectedManager, setSelectedManager] = useState(quote.user_id);
  ...
  const initialClientId = quote.client_id;
  ...
  async function onSubmit() {
    setLoading(true);
    try {
      const validationErrors = await editQuote(
        quote.id,
        clientId,
        selectedTaxId,
        selectedManager,
      ...
      await addEntityActivity(quote.id, 'quote', 'Quote edited');
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex flex-col gap-2">
          <Label htmlFor="quoteManager">Quote manager</Label>
          <Select
            onValueChange={(value) => setSelectedManager(Number(value))}
            defaultValue={quote.user_id?.toString()}
          >
            <SelectTrigger className="max-w-96 text-left" id="quoteManager">
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-2">
              <Select
                ...
                defaultValue={
                  quote.tax_id ? `${selectedTaxRate}%${selectedTaxId}` : '-1'
                }
              >
        ...
        <div className="flex max-w-96 justify-between">
          <DeleteButton
            entity={'quote'}
            deleteFunction={() => deleteQuote(quote.id)}
          />
