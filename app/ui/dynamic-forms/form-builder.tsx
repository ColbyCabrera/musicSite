...
import { Field, FieldType, Optional } from '@/app/lib/definitions';
...
export default function FormBuilder() {
  ...
  const saveForm = async () => {
    try {
      ...
      setFormName('');
      setFields([{ label: '', type: 'TEXT', defaultValue: '' }]);
    } catch (error) {
      console.error(error);
      setLoading(false);
    }
    ...
    <div>
      <div className="mb-6 flex-wrap items-stretch gap-6 md:flex">
        <div className="mb-6 min-w-52 flex-1 md:mb-0">
          <h2 className="mb-3 text-xl font-semibold">Create form</h2>
...
const PreviewField: React.FC<{ field: FieldConfig; index: number }> = ({
  field,
  index,
}) => {
