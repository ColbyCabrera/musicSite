...
import { Client, ContactFormSchema } from '@/app/lib/definitions';
...
export default function EditContactDialog({
  ...
  const form = useForm<z.infer<typeof ContactFormSchema>>({
    ...
    defaultValues: {
      ...
      phone: contact.phone_numbers?.[0]
        ...
        : '',
      phone2: contact.phone_numbers?.[1]
        ? contact.phone_numbers[1].phone_number
        : '',
      phone3: contact.phone_numbers?.[2]
        ? contact.phone_numbers[2].phone_number
        : '',
      phone4: contact.phone_numbers?.[3]
        ? contact.phone_numbers[3].phone_number
        : '',
      email: contact.emails?.[0] ? contact.emails[0].email : '',
      email2: contact.emails?.[1] ? contact.emails[1].email : '',
      email3: contact.emails?.[2] ? contact.emails[2].email : '',
      email4: contact.emails?.[3] ? contact.emails[3].email : '',
  ...
  async function onSubmit(values: z.infer<typeof ContactFormSchema>) {
    ...
    const phoneIds: number[] =
      contact.phone_numbers?.map((phoneNumber) => phoneNumber.id) ?? [];

    const emailIds: number[] = contact.emails?.map((email) => email.id) ?? [];
    ...
    if (!result?.errors) {
      setOpen(false);
      await addClientActivity(clientId, 'Edited contact');
    }
    setLoading(false);
  }
  ...
  async function deleteOnClick() {
    ...
    await addClientActivity(clientId, 'Deleted contact');
    setOpen(false);
    setDeleting(false);
  }
      ...
      <DialogContent className="block h-full max-h-screen max-w-full gap-0 overflow-y-auto sm:h-auto sm:max-w-md">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="flex grow flex-col justify-between gap-2 pt-3 pb-2">
