...
export function findClosestNote(
    ...
    allowedNotes.forEach((note) => {
        ...
        if (previousNoteMidi !== null) {
            ...
            if (direction !== 0 && targetDirection !== 0 && direction !== targetDirection) {
                score *= 2.0;
            }
            ...
            // Scoring based on interval size with direction consideration
                ...
                score *= 1.2 + (interval / avoidLeapThreshold) * (smoothnessWeight * 0.8);
                if (direction !== targetDirection) {
    ...
    if (previousNoteMidi !== null && smoothnessPref >= 7) {
        const stepNotes = allowedNotes.filter(
            (n) => Math.abs(n - previousNoteMidi!) <= 2 && 
                   n !== previousNoteMidi &&
                   Math.sign(n - previousNoteMidi!) === Math.sign(targetMidi - previousNoteMidi!)
        );

        if (stepNotes.length > 0) {
            bestNote = stepNotes.reduce((best, current) => 
                Math.abs(current - targetMidi) < Math.abs(best - targetMidi) ? current : best
            );
        }
    }
...
export function assignSopranoOrMelodyNote(
    ...
    let keyDetails = Tonal.Key.majorKey(keySignature) ?? Tonal.Key.minorKey(keySignature);
    if (!keyDetails) {
    ...
    const diatonicPitchClasses = new Set(scaleNotes.map(note => Tonal.Note.chroma(note)));
    ...
    if (previousNote !== null) {
        ...
        for (const note of preferredRange) {
            if (note >= minRange && note <= maxRange) {
                ...
                if (diatonicPitchClasses.has(note % 12) && !fullChordNotePool.includes(note)) {
        ...
        if (Math.random() < 0.05) { // 5% chance to even consider chromatic notes
            for (const note of preferredRange) {
                if (note >= minRange && note <= maxRange &&
        ...
        if (availableNotes.includes(previousNote) && Math.random() < oscillationPenalty) {
        ...
        availableNotes = fullChordNotePool.filter(n => n >= minRange && n <= maxRange);
        
        // Prefer starting in the middle register
        const midpoint = (minRange + maxRange) / 2;
        availableNotes.sort((a, b) => 
            Math.abs(a - midpoint) - Math.abs(b - midpoint)
        );
    }
