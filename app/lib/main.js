...
async function handleGenerateClick() {
  ...
  // IMPORTANT: Get the sheet-music div specifically for clearing/status
  const sheetMusicContainer = document.getElementById("sheet-music");
  if (sheetMusicContainer) {
    sheetMusicContainer.innerHTML = "<p>Generating...</p>"; // Clear sheet music area
  } else {
    console.warn("Could not find sheet-music container to clear.");
  }

  if (isPlaying) {
    // Stop playback if generating new music
  ...
  setTimeout(() => {
    try {
      ...
      // 3. Display Music (pass the specific container element)
      const sheetMusicContainer = document.getElementById("sheet-music");
      if (!sheetMusicContainer) {
        throw new Error("Sheet music container not found in DOM.");
      }
      ...
      // Display error in the sheet music container as well
      const sheetMusicContainer = document.getElementById("sheet-music");
      if (sheetMusicContainer) {
        sheetMusicContainer.innerHTML = `<p style="color:red;">Error: ${error.message}<br>Check console.</p>`;
      }
...
// --- Initialize Function ---
// This function now contains the logic that should run once the module is imported
// and potentially after other setup (like font loading) is done.
function initializeApp() {
  console.log("Initializing application...");

  // Run UI initialization which includes getting DOM references etc.
  initializeUI();

  // Attach Event Listeners (Ensure buttons exist first, initializeUI should handle this)
  if (generateBtn) {
    generateBtn.addEventListener("click", handleGenerateClick);
    console.log("Attached listener to Generate button.");
  } else {
    console.error("Generate button not found during initialization!");
  }

  if (playBtn) {
    playBtn.addEventListener("click", handlePlayClick);
    console.log("Attached listener to Play button.");
  } else {
    console.error("Play button not found during initialization!");
  }

  if (stopBtn) {
    stopBtn.addEventListener("click", handleStopClick);
    console.log("Attached listener to Stop button.");
  } else {
    console.error("Stop button not found during initialization!");
  }

  console.log("Main application initialization complete.");
  updateStatus("Ready. Select settings and click Generate."); // Set initial ready status
}

// --- Export the Initializer ---
export { initializeApp }; // <-- Make the function available for import
