 ...
 */
function findClosestNote(targetMidi, allowedNotes, previousNoteMidi, smoothnessPref, avoidLeapThreshold = Tonal.Interval.semitones("P5")) {
    ...
    allowedNotes.forEach((note) => {
        ...
        if (previousNoteMidi !== null) {
            ...
            if (interval === 0) { score *= 0.1 * (1.1 - smoothnessWeight); }
            else if (interval <= 2) { score *= 0.5 * (1.1 - smoothnessWeight); }
            else if (interval <= avoidLeapThreshold) { score *= 1.0 + (interval / avoidLeapThreshold) * (smoothnessWeight * 0.5); }
            else { score *= 1.5 + (interval / 12.0) * smoothnessWeight; }
        }
        if (score < minScore) {
            minScore = score;
            bestNote = note;
    ...
    if ( previousNoteMidi !== null && Math.abs(bestNote - previousNoteMidi) > avoidLeapThreshold) {
 ...
 */
function assignBassNote(chordRootMidi, chordNotesPool, previousBassMidi, smoothness) {
    ...
    if (rootOptions.length > 0) {
        ...
        return findClosestNote(targetMidi, rootOptions, previousBassMidi, smoothness);
    } else {
        console.log(`Root note (${Tonal.Note.pitchClass(Tonal.Note.fromMidi(chordRootMidi))}) not available in bass range.`);
        const targetMidi = previousBassMidi ?? chordRootMidi - 12;
        return findClosestNote(targetMidi, allowedBassNotes, previousBassMidi, smoothness);
    }
}
 ...
 */
function assignSopranoNote(fullChordNotePool, previousSopranoMidi, previousAltoMidi, smoothness) {
    // ... (Keep the full implementation of assignSopranoNote here) ...
     ...
     return findClosestNote(targetMidi, allowedSopranoNotes, previousSopranoMidi, smoothness);
}
 ...
 */
function assignInnerVoices(tenorTargetNotePc, altoTargetNotePc, fullChordNotePool, previousTenorMidi, previousAltoMidi, sopranoNoteMidi, bassNoteMidi, smoothness) {
    // ... (Keep the full implementation of assignInnerVoices here) ...
    let allowedAltoNotes = fullChordNotePool.filter(
        (n) => n >= VOICE_RANGES.alto[0] && n <= VOICE_RANGES.alto[1] &&
               n < sopranoNoteMidi && n > bassNoteMidi &&
               Math.abs(sopranoNoteMidi - n) <= VOICE_SPACING_LIMIT.soprano_alto
    );
    let allowedTenorNotes = fullChordNotePool.filter(
        (n) => n >= VOICE_RANGES.tenor[0] && n <= VOICE_RANGES.tenor[1] &&
               n < sopranoNoteMidi && n > bassNoteMidi &&
    ...
    let altoTargetMidi = previousAltoMidi ? previousAltoMidi + (Math.random() > 0.5 ? 1 : -1) : (sopranoNoteMidi + bassNoteMidi) / 2;
    const altoTargetPcOptions = allowedAltoNotes.filter(n => n % 12 === altoTargetNotePc);
    if (altoTargetPcOptions.length > 0) {
        altoNoteMidi = findClosestNote(altoTargetMidi, altoTargetPcOptions, previousAltoMidi, smoothness);
    }
    if (altoNoteMidi === null) {
        if (allowedAltoNotes.length === 0) {
            console.warn("No valid notes for Alto. Using fallback.");
            altoNoteMidi = previousAltoMidi ?? Math.max(VOICE_RANGES.alto[0], bassNoteMidi + 1);
        } else {
            altoNoteMidi = findClosestNote(altoTargetMidi, allowedAltoNotes, previousAltoMidi, smoothness);
        }
    }

    // Tenor
    allowedTenorNotes = allowedTenorNotes.filter(
        (n) => n < altoNoteMidi && Math.abs(altoNoteMidi - n) <= VOICE_SPACING_LIMIT.alto_tenor
    );
    let tenorNoteMidi = null;
    let tenorTargetMidi = previousTenorMidi ? previousTenorMidi + (Math.random() > 0.5 ? 1 : -1) : (altoNoteMidi + bassNoteMidi) / 2;
    const tenorTargetPcOptions = allowedTenorNotes.filter(n => n % 12 === tenorTargetNotePc);
    if (tenorTargetPcOptions.length > 0) {
        tenorNoteMidi = findClosestNote(tenorTargetMidi, tenorTargetPcOptions, previousTenorMidi, smoothness);
    }
    if (tenorNoteMidi === null) {
        if (allowedTenorNotes.length === 0) {
            console.warn("No valid notes for Tenor below Alto. Using fallback.");
            tenorNoteMidi = previousTenorMidi ?? Math.max(VOICE_RANGES.tenor[0], bassNoteMidi + 1);
            if (tenorNoteMidi >= altoNoteMidi) { tenorNoteMidi = altoNoteMidi - 1; }
        } else {
            tenorNoteMidi = findClosestNote(tenorTargetMidi, allowedTenorNotes, previousTenorMidi, smoothness);
        }
    }

    // Final check
    if (tenorNoteMidi >= altoNoteMidi) {
        console.warn(`Tenor (${midiToNoteName(tenorNoteMidi)}) >= Alto (${midiToNoteName(altoNoteMidi)}). Correcting.`);
        const lowerTenorOptions = allowedTenorNotes.filter((n) => n < altoNoteMidi);
        if (lowerTenorOptions.length > 0) {
            tenorNoteMidi = findClosestNote(tenorTargetMidi, lowerTenorOptions, previousTenorMidi, smoothness);
        } else {
            tenorNoteMidi = altoNoteMidi - (Tonal.Note.semitones(midiToNoteName(altoNoteMidi) % 12) % 2 === 0 ? 2 : 1);
            tenorNoteMidi = Math.max(tenorNoteMidi, bassNoteMidi + 1, VOICE_RANGES.tenor[0]);
 ...
 * (Copied from original, kept internal)
 */
function checkParallels(voice1Prev, voice1Curr, voice2Prev, voice2Curr, part1Name, part2Name, measure, beat) {
    // ... (Keep the full implementation of checkParallels here) ...
     if (voice1Prev === null || voice2Prev === null || voice1Curr === null || voice2Curr === null) return;
     ...
     if (!note1PrevName || !note1CurrName || !note2PrevName || !note2CurrName) return;
     ...
     if (voice1Moved && voice2Moved) {
         ...
         if (numPrev === 5 && numCurr === 5 && simplePrev === 'P5' && simpleCurr === 'P5') {
             console.warn(`PARALLEL 5th (${part1Name}/${part2Name}) at ${loc}. Prev: ${note1PrevName}-${note2PrevName}, Curr: ${note1CurrName}-${note2CurrName}`);
         } else if ((numPrev === 8 || numPrev === 1) && (numCurr === 8 || numCurr === 1) && simplePrev.startsWith('P') && simpleCurr.startsWith('P')) {
             console.warn(`PARALLEL Octave/Unison (${part1Name}/${part2Name}) at ${loc}. Prev: ${note1PrevName}-${note2PrevName}, Curr: ${note1CurrName}-${note2CurrName}`);
         }
     }
}
...
function checkVoiceLeadingRules(currentNotes, previousNotes, measure, beat) {
    ...
    // Crossing
    if (alto > soprano) console.warn(`Voice Crossing: Alto (${midiToNoteName(alto)}) > Soprano (${midiToNoteName(soprano)}) at ${loc}`);
    if (tenor > alto)   console.warn(`Voice Crossing: Tenor (${midiToNoteName(tenor)}) > Alto (${midiToNoteName(alto)}) at ${loc}`);
    if (bass > tenor)   console.warn(`Voice Crossing: Bass (${midiToNoteName(bass)}) > Tenor (${midiToNoteName(tenor)}) at ${loc}`);

    // Spacing
    if (Math.abs(soprano - alto) > VOICE_SPACING_LIMIT.soprano_alto) console.warn(`Spacing > P8 between Soprano/Alto at ${loc}`);
    if (Math.abs(alto - tenor) > VOICE_SPACING_LIMIT.alto_tenor)     console.warn(`Spacing > P8 between Alto/Tenor at ${loc}`);
    if (Math.abs(tenor - bass) > VOICE_SPACING_LIMIT.tenor_bass)     console.warn(`Spacing > P12 between Tenor/Bass at ${loc}`);

    // Parallels
    if (prev && prev.soprano !== null) {
        checkParallels(prev.soprano, soprano, prev.alto, alto, "Soprano", "Alto", measure, beat);
        checkParallels(prev.soprano, soprano, prev.tenor, tenor, "Soprano", "Tenor", measure, beat);
        checkParallels(prev.soprano, soprano, prev.bass, bass, "Soprano", "Bass", measure, beat);
        checkParallels(prev.alto, alto, prev.tenor, tenor, "Alto", "Tenor", measure, beat);
        checkParallels(prev.alto, alto, prev.bass, bass, "Alto", "Bass", measure, beat);
        checkParallels(prev.tenor, tenor, prev.bass, bass, "Tenor", "Bass", measure, beat);
    }
}
     ...
     for (let i = 1; i < numMeasures - 1; i++) {
         ...
         do {
             ...
             if ([subdominantRoman, supertonicRoman].includes(prevChord)) {
                 const dominantCandidates = allowedChords.filter((c) => [dominantRoman, dominant7Roman, leadingToneRoman].includes(c));
                 if (dominantCandidates.length > 0) candidates = dominantCandidates;
             } else if ([dominantRoman, dominant7Roman].includes(prevChord)) {
                 const tonicResolutionCandidates = allowedChords.filter((c) => [tonicRoman, submediantRoman].includes(c));
                 if (tonicResolutionCandidates.length > 0) candidates = tonicResolutionCandidates;
             } else if (prevChord === submediantRoman) {
                 const submediantNextCandidates = allowedChords.filter(c => [supertonicRoman, subdominantRoman, dominantRoman, dominant7Roman].includes(c));
                 if (submediantNextCandidates.length > 0) candidates = submediantNextCandidates;
             }
             if (candidates.length === 0) candidates = allowedChords;
             nextChord = candidates[Math.floor(Math.random() * candidates.length)];
             attempts++;
         } while (nextChord === prevChord && allowedChords.length > 1 && attempts < MAX_ATTEMPTS);
         progression.push(nextChord);
         prevChord = nextChord;
     }

     const preCadenceChord = allowedChords.includes(dominant7Roman) ? dominant7Roman : dominantRoman;
     if (numMeasures > 2) {
         if (allowedChords.includes(preCadenceChord)) { progression[numMeasures - 2] = preCadenceChord; }
         else if (allowedChords.includes(dominantRoman)) { progression[numMeasures - 2] = dominantRoman; }
 ...
 */
export function generateVoices(chordProgression, key, meter, numMeasures, generationSettings) {
    // ... (Keep the full implementation of generateVoices here, calling internal helpers) ...
    ...
    const leadingToneMidiPc = (Tonal.Note.midi(keyTonic + DEFAULT_OCTAVE) + Tonal.Interval.semitones("M7")) % 12;

    const [meterBeats, beatValue] = meter.split("/").map(Number);
    if (![2, 4, 8, 16].includes(beatValue)) throw new Error("Unsupported beat value: " + meter);

    let voicesData = { soprano: [], alto: [], tenor: [], bass: [] };
    let previousNotesMidi = { soprano: null, alto: null, tenor: null, bass: null };

    const chordDurationStr = { 1: 'w', 2: 'h', 4: 'q', 8: '8' }[beatValue] || 'q';
    ...
    for (let i = 0; i < numMeasures; i++) {
        ...
        if (baseChordNotes.length === 0) {
            console.error(`Skipping measure ${i + 1}: Chord error "${roman}" in ${key}. Adding rests.`);
            VOICE_ORDER.forEach((voiceName) => {
                voicesData[voiceName].push({ midi: null, duration: restDurationStr, vexKey: "b/4", isRest: true });
            });
            previousNotesMidi = { soprano: null, alto: null, tenor: null, bass: null };
            continue;
        }
        ...
        // Assign Notes
        const bassNoteMidi = assignBassNote(chordRootMidi, fullChordNotePool, previousNotesMidi.bass, melodicSmoothness);
        const sopranoNoteMidi = assignSopranoNote(fullChordNotePool, previousNotesMidi.soprano, previousNotesMidi.alto, melodicSmoothness);

        // Determine Inner Voice PCs
        ...
        if (neededPcs.length < voicesToFill) {
            ...
            const canDoubleRoot = chordRootPc !== leadingToneMidiPc;
             if (canDoubleRoot && !currentVoicingPcs.has(chordRootPc)) { pcsToDouble.push(chordRootPc); }
            const fifthPcMidi = Tonal.Note.midi(Tonal.Note.transpose(Tonal.Note.fromMidi(chordRootMidi), "P5")) % 12;
             if (pcsToDouble.length < numDoublingsNeeded && chordPcs.includes(fifthPcMidi) && fifthPcMidi !== leadingToneMidiPc && !currentVoicingPcs.has(fifthPcMidi) && !neededPcs.includes(fifthPcMidi)) { pcsToDouble.push(fifthPcMidi); }
            const thirdPc = chordPcs.find(pc => pc !== chordRootPc && pc !== fifthPcMidi);
             if (pcsToDouble.length < numDoublingsNeeded && thirdPc !== undefined && thirdPc !== leadingToneMidiPc && !currentVoicingPcs.has(thirdPc) && !neededPcs.includes(thirdPc)) { pcsToDouble.push(thirdPc); }
             while (pcsToDouble.length < numDoublingsNeeded) {
                 if (!currentVoicingPcs.has(chordRootPc) && !neededPcs.includes(chordRootPc) && !pcsToDouble.includes(chordRootPc)) { pcsToDouble.push(chordRootPc); }
                 else if (chordPcs.includes(fifthPcMidi) && !currentVoicingPcs.has(fifthPcMidi) && !neededPcs.includes(fifthPcMidi) && !pcsToDouble.includes(fifthPcMidi)) { pcsToDouble.push(fifthPcMidi); }
                 else if (thirdPc !== undefined && !currentVoicingPcs.has(thirdPc) && !neededPcs.includes(thirdPc) && !pcsToDouble.includes(thirdPc)) { pcsToDouble.push(thirdPc); }
                 else { pcsToDouble.push(chordRootPc); }
                 if (pcsToDouble.length > voicesToFill*2) break; // Safety break
             }
         }
        let targetInnerPcs = [...neededPcs, ...pcsToDouble].slice(0, voicesToFill);
        while (targetInnerPcs.length < voicesToFill) { // Fallback fill
             const fifthPcMidi = Tonal.Note.midi(Tonal.Note.transpose(Tonal.Note.fromMidi(chordRootMidi), "P5")) % 12;
             if (!targetInnerPcs.includes(chordRootPc)) { targetInnerPcs.push(chordRootPc); }
             else if (chordPcs.includes(fifthPcMidi) && !targetInnerPcs.includes(fifthPcMidi)) { targetInnerPcs.push(fifthPcMidi); }
             else { targetInnerPcs.push(chordRootPc); }
        }
        const tenorTargetPc = targetInnerPcs[0];
        const altoTargetPc = targetInnerPcs[1];

        const { tenorNoteMidi, altoNoteMidi } = assignInnerVoices(tenorTargetPc, altoTargetPc, fullChordNotePool, previousNotesMidi.tenor, previousNotesMidi.alto, sopranoNoteMidi, bassNoteMidi, melodicSmoothness);

        // Consolidate & Check
        const currentStepNotes = { soprano: sopranoNoteMidi, alto: altoNoteMidi, tenor: tenorNoteMidi, bass: bassNoteMidi };
        ...
        VOICE_ORDER.forEach((voiceName) => {
            ...
            if (midi !== null && vexKey) {
                voicesData[voiceName].push({ midi: midi, duration: chordDurationStr, vexKey: vexKey, isRest: false });
            } else {
                console.warn(`Assigning rest for ${voiceName} at M${i + 1}`);
                voicesData[voiceName].push({ midi: null, duration: restDurationStr, vexKey: "b/4", isRest: true });
                currentStepNotes[voiceName] = null;
            }
        });
